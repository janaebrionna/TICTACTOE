class Game {
    ...
    drawMessage(){
        const message = `Game eended in a draw`
        return this.gameStatus.innerText = message
    }
    ...
    handleCellPlayed(cell, idx) {
        ...
        this.currentPlayer =='X' ? cell.classList.add('red') : cell.classList.add('pink')
    ...
    resultValidation() {
        ...
        if (gameWon) {
            ...
            if (winner =='X'){
                ...
                this.xWins.innerHTML +=`<span class="tally"> ${tallyMark}
                </span>`
            } else{
                this.winCount.o = this.winCount.o + 1
                this.oWins.innerHTML += `<span class="tally"> ${tallyMark}
                </span>`
            }
            this.checkWinCount()
